...
export default function Home() {
    ...
    useEffect(() => {
        const getAssessmentData = async () => {
            ...
            const userData = await (currentUserEmail
                ...
                : []);
            const sortedUserData = await (userData.sort((a,b) => new Date(b.assessDate) - new Date(a.assessDate)))
            setAssessmentData(sortedUserData);

            // checking if there are any assessments for the current month
            const dateCheck = sortedUserData.length > 0 ? sortedUserData.filter((user) => new Date(user.assessDate).getMonth() === currentDate) : []

            // fetching data for each category
            setDeprData(
                dateCheck.length > 0
                    ? dateCheck.filter((d) => d.hasOwnProperty(
                          "depressionLevel"
                      ))
                    : 0
            );
            ...
            const userPersonality = await (currentUserId ? personalityAssess.filter((user) => user.userId === currentUserId) : [])
            const sortedPersonality = await (userPersonality.length > 0 ? userPersonality.sort((a,b) => new Date(b.createdAt) - new Date(a.createdAt)) : [])
            setRecentPersonalityType(sortedPersonality.length > 0 ? sortedPersonality[0].personalityType : '')
        };
        getAssessmentData();
    ...
    const fetchPersonality = async () => {
        const res = await fetch("http://localhost:3000/api/personality");
        const data= await res.json();
    ...
    return (
        <div className="grid grid-cols-4 gap-1 bg-slate-200">
            ...
            <div className="col-span-3">
                ...
                <div className="m-2 rounded-lg bg-white p-2">
                    <div className="flex justify-between">
                        <h2 className="font-bold pb-2">Your Assessment History</h2>
                        <button onClick={() => setPopup(true)}>View all</button>
                    </div>
                    {assessmentData.length > 0 ? (
                        <AssessmentHistory assessment={assessmentData.slice(0,6)} />
                    ) : (
                        <NoAssessResult />
                    )}
                </div>
                ...
                <div className="m-2 rounded-lg bg-white p-2">
                    <div className="flex justify-between pb-4">
                        ...
                        <Link href={`http://localhost:3000/education`}>View all</Link>
                    </div>
            ...
            <PopUpAssessmentHistory isVisible={popup} onClose={() => setPopup(false)} assessment={assessmentData} />
        </div>
    );
}
